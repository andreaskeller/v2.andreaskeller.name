...
const mdxComponents = {
  a: ({ className, ...props }) => (
    <a
      className={`bg-blue-50 hover:bg-blue-200 border-blue-400 border-b-2 no-underline`}
      {...props}
    />
  ),
};
...
export default function Post({ meta, children, posts }) {
  ...
  return (
    <article className="xl:divide-y xl:divide-gray-200">
      <Head>
        <title>{meta.title} – Andreas Keller</title>
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:site" content="@itsakeller" />
        <meta name="twitter:creator" content="@itsakeller" />
        <meta name="twitter:title" content={`${meta.title} – Andreas Keller`} />
        <meta name="twitter:description" content={meta.description} />
        <meta name="twitter:image" content={`https://andreaskeller.name${meta.image}`} />
        <meta property="og:url" content={`https://andreaskeller.name${router.pathname}`} />
        <meta property="og:type" content="article" />
        <meta property="og:title" content={`${meta.title} – Andreas Keller`} />
        <meta property="og:description" content={meta.description} />
        <meta property="og:image" content={`https://andreaskeller.name${meta.image}`} />
            ...
            <div className="space-y-8 py-8">
              {next && (
                <div>
                  ...
                  <div>
                    <Link href={next.link}>
                      <a className="bg-blue-50 hover:bg-blue-200 border-blue-400 border-b-2">
                        {next.title}
                      </a>
                    </Link>
                  </div>
